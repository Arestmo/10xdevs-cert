...
await supabase.from("generation_events").insert(events);
```
...
const sanitizedText = source_text.trim().replace(/[^\w\s\.,!?;:()\-]/g, ""); // Keep only safe characters
...
await supabase.from("profiles").select("*").eq("user_id", user.id); // Parameter binding
...
// Usage
return errorResponse("AI_LIMIT_EXCEEDED", "Monthly AI generation limit exceeded", 403, {
  current_count: 200,
  limit: 200,
});
```
...
await supabase.from("generation_events").insert(events); // Array of events
